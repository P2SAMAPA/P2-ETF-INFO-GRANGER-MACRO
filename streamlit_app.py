...
st.set_page_config(page_title="Info‑Theoretic Granger Causality with Macro Adjustment", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🔄 Info‑Theoretic Granger Causality</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Transfer entropy with macro‑conditioning | Net information outflow as lead‑lag score</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Info Granger")
...
st.sidebar.markdown(f"**Bins:** {config.N_BINS} | **Lag:** {config.LAG} | **Permutations:** {config.NUM_PERMUTATIONS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'info_granger_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Net info outflow: {etf['granger_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Net Outflow"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Net Outflow", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🔄 Top ETFs by Net Information Outflow (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Transfer entropy** measures directed information flow (non‑linear Granger causality).
        - We condition on macro variables (VIX, DXY, yields) to remove spurious causality.
        - A permutation test determines statistical significance.
        - The per‑ETF score is the **net information outflow** = sum(outgoing TE) − sum(incoming TE) for significant edges.
        - Positive net outflow → ETF leads others (influencer).
        - Negative net outflow → ETF follows others (follower).
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the net information outflow.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Info‑Theoretic Granger Causality | Macro‑conditioned transfer entropy")
